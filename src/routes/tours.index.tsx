  ...
  head: () => ({
    meta: [
      { title: "Tours — EDGELINK Tours" },
      {
        name: "description",
        content:
          "Browse every EDGELINK Tours expedition: gorilla trekking, Big Five safaris, rainforest canopy walks and Lake Kivu retreats.",
      },
...
function ToursIndex() {
  ...
  const media = useMediaUrls(
    "tours",
    (tours ?? []).map((t) => t.featured_image_url),
  );

  const regions = useMemo(
    () => ["All", ...new Set((tours ?? []).map((t) => t.region || t.location).filter(Boolean))],
    [tours],
  );
  const durations = useMemo(
    () => ["All", ...new Set((tours ?? []).map((t) => t.duration).filter(Boolean))],
    [tours],
  );
  const activities = useMemo(
    () => ["All", ...new Set((tours ?? []).map((t) => t.activity).filter(Boolean) as string[])],
    [tours],
  );
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:px-6">
        <div className="mb-8 space-y-3">
          ...
          <p className="pt-1 text-xs text-muted-foreground">
            {tr("hub.results", { count: items.length })}
          </p>
        </div>
